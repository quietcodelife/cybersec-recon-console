...
def run():
    while True:
        ...
        try:
            ...
            try:
                sitemap_result, tls_notice = safe_fetch(sitemap_url)
                if tls_notice:
                    tls_notices.append(tls_notice)
            except urllib.error.HTTPError as exc:
                sitemap_error = f"{exc.code} {exc.reason}"
            except urllib.error.URLError as exc:
                sitemap_error = str(exc.reason)
            ...
            print(f" SITEMAP.XML:    {G + str(sitemap_result['status']) + RESET if sitemap_result else R + 'UNAVAILABLE' + RESET}")
            print(" ----------------------------------------------------------------")
            ...
            if sitemap_result:
                ...
                print(f"\n {R}[SITEMAP.XML]{RESET} {sitemap_error or 'Not available'}")

            report_lines = [
                f"ROBOTS / SITEMAP RECON: {base_url}",
                f"robots.txt: {robots_result['status'] if robots_result else 'UNAVAILABLE'}",
                f"sitemap.xml: {sitemap_result['status'] if sitemap_result else 'UNAVAILABLE'}",
            ]
            ...
            if sitemap_result:
                ...
                report_lines.extend(["", f"[sitemap.xml] {sitemap_error or 'Not available'}"])

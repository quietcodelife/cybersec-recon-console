...
AUXILIARY_PREFIXES = ("utun", "awdl", "llw", "gif", "stf", "anpi", "ap", "bridge")
...
def parse_gateway():
    result = safe_run(["route", "-n", "get", "default"])
    if not result:
        return "No data"
    match = re.search(r"gateway:\s+([0-9.]+)", result.stdout or "")
    return match.group(1) if match else "No data"


def parse_dns_servers():
    result = safe_run(["scutil", "--dns"])
    if not result:
        return []
    resolvers = []
...
def ping_target(target):
    if not target or target == "No data":
        return None
    result = safe_run(["ping", "-c", "1", target])
    if not result:
        return None
